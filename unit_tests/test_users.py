...
def test_change_user_password(client):
    response = client.patch('/api/users/dkfhjk32ds62hj2d323d', json={"password": "new_password"})
...
def test_add_user_to_project(mock_create_sns_subscription, client):
    response = client.post('/api/projects/fdlkj432987jh43hjkds/users', json={"user_uuid": 'dkfhjk32ds62hj2d323d'})
...
def test_remove_user_from_project(mock_remove_sns_subscription, client):
    response = client.delete('api/projects/fdlkj432987jh43hjkds/users/dkfhjk32ds62hj2d323d')

    assert response.status_code == status_codes.HTTP_204_NO_CONTENT

def test_cannot_add_root_user_to_project(client):
    response = client.post('api/projects/fdlkj432987jh43hjkds/users', json={"user_uuid": "root-uuid-123-456-789"})
    response_json = response.get_json()
