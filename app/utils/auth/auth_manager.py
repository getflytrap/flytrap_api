...
class AuthManager:
    ...
    def authenticate(self, f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                current_app.logger.debug(f"Authentication successful for token: {token}.")
                ...
                current_app.logger.error(f"Unexpected error during token authentication: {e}", exc_info=True)
    ...
    def authorize_root(self, f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            current_app.logger.info("Authorization failed: User lacks root permissions.")
    ...
    def authorize_project_access(self, f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not project_uuid:
                current_app.logger.error("Authorization failed: Missing project_uuid in request.")
                return jsonify({"message": "Project identifier is required."}), 400

            # Allow root users universal access
            if is_root:
                current_app.logger.debug(f"Root access granted for project UUID={project_uuid}.")
                return f(*args, **kwargs)
            ...
            try:
                ...
                if user_uuid in project_users:
                    current_app.logger.debug(f"Access granted to user UUID={user_uuid} for project UUID={project_uuid}.")
                    return f(*args, **kwargs)

                current_app.logger.info(f"Authorization failed: User UUID={user_uuid} not assigned to project UUID={project_uuid}.")
                return jsonify({"message": "You do not have the necessary permissions to perform this action."}), 403
            except Exception as e:
                current_app.logger.error(f"Unexpected error during project authorization: {e}", exc_info=True)
                return jsonify({"message": "Internal server error."}), 500
    ...
    def authorize_user(self, f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if current_user_uuid == user_uuid_in_path:
                current_app.logger.debug(f"Access granted for user UUID={current_user_uuid}.")
                return f(*args, **kwargs)
            
            current_app.logger.info(f"Authorization failed: User UUID={current_user_uuid} attempted to access another user's settings.")
            return jsonify({"message": "You do not have the necessary permissions to perform this action."}), 403
