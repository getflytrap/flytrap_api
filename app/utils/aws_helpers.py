...
def create_sns_topic(project_uuid: str) -> str:
    ...
    if current_app.config["ENVIRONMENT"] == "development":
        current_app.logger.info(
            f"[MOCK] Creating sns topic arn for project {project_uuid}."
        )
        return (
    ...
    try:
        ...
        current_app.logger.error(
            f"Failed to create SNS topic for project {project_uuid}: {e}"
        )
        raise RuntimeError(
...
def send_sns_notification(project_uuid: str) -> None:
    ...
    if current_app.config["ENVIRONMENT"] == "development":
        current_app.logger.info(
            f"[MOCK] Sending notification to {project_uuid}: New Issue Logged"
        )
        return
...
def delete_sns_topic_from_aws(project_uuid: str) -> None:
    ...
    if current_app.config["ENVIRONMENT"] == "development":
        current_app.logger.info(
            f"[MOCK] Deleting SNS topic for project {project_uuid}."
        )
        return
