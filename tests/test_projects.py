...
def test_get_projects(client):
    ...
    # the route handler's response should include the projects fetched from the
    # db in its payload
    print("repsonse json", response_json)
    assert response_json["payload"]["projects"] == [
        MOCK_DATA["fetch_projects"]["projects"][0]
    ]
...
def test_create_project(
    mock_generate_uuid, mock_create_sns, mock_associate_api_key, client
):

    # the generate_uuid method is used twice, once to generate the project_uuid, once
    # for the api_key
    mock_generate_uuid.side_effect = ["dajhew32876dcx79sd2332", "api_key_123"]
    mock_create_sns.return_value = "986723jgds23wkhdskh32"

    response = client.post(
        "/api/projects", json={"name": "testing123", "platform": "react"}
    )
...
def test_delete_project(mock_delete_sns_topic, mock_delete_api_key, client):
    ...
    # the delete_api_key AWS service call should be passed the deleted project's
    # api key as an argument
...
def test_get_projects_no_results(client):
    ...
    """
    with patch(
        "app.routes.projects.fetch_projects",
        return_value={
            "projects": [],
            "total_pages": 1,
            "current_page": 1,
        ...
        response = client.get(
            "/api/projects", query_string={"page": "1", "limit": "10"}
        )
        response_json = response.get_json()
